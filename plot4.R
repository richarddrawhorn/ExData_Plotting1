...
# set 2x2 size to contain the four graphs
par(mfrow=c(2,2))

with(data, {
        # first plot (upper left)
        plot(dt, Global_active_power, xlab="Day/Time", type="l", ylab="Global Active Power (kilowatts)")
        # second plot (upper right)
        plot(dt, Voltage, xlab="Day/Time", type="l", ylab="Voltage (Volts)")
        # third plot (lower left)
        plot(dt, Sub_metering_1, ylab="Energy sub metering (watt-hour of active energy)", xlab="Day/Time", type="l")
        ...
        plot(dt, Global_reactive_power, xlab="Day/Time", type="l", ylab="Global Reactive Power (kilowatts)")
})

title("Plot 4", outer=TRUE)
